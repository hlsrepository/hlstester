...
extern void bfs(int, int*, int*, std::vector<int>&, int*, int*, int*, int*);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < testCases.size(); i++) {
        ...
        bfs(testCases[i].input.size(), testCases[i].input.data(), results.data(), feedbackOutput, &fallback, &feedbackValue, &recursionResult, &diff);
